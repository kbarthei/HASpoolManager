// docs-coverage: ignore — admin manual snapshot trigger, internal helper
...
export async function POST(request: NextRequest) {
  ...
  const latest = await db.query.prints.findFirst({
    orderBy: [desc(prints.finishedAt)],
  });
  if (!latest) return NextResponse.json({ error: "no print found" }, { status: 404 });

  await db.update(prints).set({ snapshotPath: path, updatedAt: new Date() })
